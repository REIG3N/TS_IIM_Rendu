...
interface Task { // création de l'interface de Task
  id: number;
  title: string;
  description: string;
  deadline: Date;
  priority: 'Low' | 'Medium' | 'High'; // Priorité de la tâche au choix pour l'utilisateur
  category: string;
}

class TaskManager {
  tasks: Task[] = []; // On créer un tableau des tâches vide pour le moment
  ...
  constructor( private categoryManager: CategoryManager) {
    ...
    */
  addTask(task: Task, loading: boolean): void {
    ...
    if (!loading) {  // Si la tâche est ajoutée directement (pas en cours de chargement)
      ...
      if (tasksContainer) {  // Si des tâches sont déjà présentes dans le tasksContainer
        ...
        if (filteredTasks) {     
          ...
          const parsedFilteredTasks: Task[] = JSON.parse(filteredTasks); // On transforme en tableau les taches filtrees provenant du localStorage
...
filterTasksByPriority(priority: 'Low' | 'Medium' | 'High'): void {
...
filterTasksByDate(date: string): void {
...
function CreateNewTask(event: Event) {
  ...
  const dueDateInput = document.getElementById('taskDueDate') as HTMLInputElement;
  const priorityInput = document.getElementById('taskPriority') as HTMLSelectElement;
  const categoryInput = document.getElementById('taskCategory') as HTMLSelectElement;
...
/**
 * -------------------------------------------------------------  Création et appel des Categories  -------------------------------------------------------------
 */
...
document.addEventListener('DOMContentLoaded', () => {
  const categorySelect = document.getElementById('filterByCategory') as HTMLSelectElement;
  const categoryManager = new CategoryManager();
  const categories = categoryManager.getAllCategories();
